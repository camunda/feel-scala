...
/** @author
  *   Philipp Ossler
  */
class InterpreterListExpressionTest extends AnyFlatSpec with Matchers with FeelIntegrationTest {
  ...
  it should "be processed in a for-expression" in {
    ...
    eval("for x in [1,2] return x * 2") should be(ValList(List(ValNumber(2), ValNumber(4))))
  ...
  it should "be filtered via comparison" in {
    ...
    eval("[1,2,3,4][item > 2]") should be(ValList(List(ValNumber(3), ValNumber(4))))
  ...
  it should "be filtered via boolean expression" in {
    eval("[1,2,3,4][odd(item)]") should be(ValList(List(ValNumber(1), ValNumber(3))))

    eval("[1,2,3,4][even(item)]") should be(ValList(List(ValNumber(2), ValNumber(4))))
  }
  ...
  it should "be filtered multiple times (in a context)" in {
    ...
    eval("{z: x.y[1][1][1][1]}.z", Map("x" -> Map("y" -> List(List(listOfLists))))) should be(
      ValNumber(1)
    )
  }
  ...
  it should "fail if one element fails" in {
    ...
    eval("[1, {}.x]") should be(ValError("context contains no entry with key 'x'"))
  }
  ...
  it should "access the partial result" in {
    ...
    eval("for x in 1..5 return if (x = 1) then 1 else x + sum(partial)") should be(
      ValList(List(ValNumber(1), ValNumber(3), ValNumber(7), ValNumber(15), ValNumber(31)))
    )

    eval("for i in 1..8 return if (i <= 2) then 1 else partial[-1] + partial[-2]") should be(
      ValList(
        List(
          ValNumber(1),
          ValNumber(1),
  ...
  it should "be checked with 'some'" in {
    eval("some x in xs satisfies x >= 10000", Map("xs" -> hugeList)) should be(ValBoolean(true))

    eval("some x in xs satisfies x > 10000", Map("xs" -> hugeList)) should be(ValBoolean(false))
  }

  it should "be checked with 'some' (invalid condition)" in {
    eval("some x in xs satisfies null", Map("xs" -> hugeList)) should be(ValNull)
  }

  it should "be checked with 'every'" in {
    eval("every x in xs satisfies x > 0", Map("xs" -> hugeList)) should be(ValBoolean(true))
  }

  it should "be checked with 'every' (invalid condition)" in {
    eval("every x in xs satisfies null", Map("xs" -> hugeList)) should be(ValNull)
  }
