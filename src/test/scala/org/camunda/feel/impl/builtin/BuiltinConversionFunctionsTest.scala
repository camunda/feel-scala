...
class BuiltinConversionFunctionsTest
  ...
  "A to json() function" should "convert a string value" in {
    evaluateExpression(""" to json("hello") """) should returnResult(
      "\"hello\""
    )
  }

  it should "convert a number" in {
    evaluateExpression(""" to json(42) """) should returnResult(
      "42"
    )
  }

  it should "convert a boolean" in {
    evaluateExpression(""" to json(true) """) should returnResult(
      "true"
    )
  }

  it should "convert a list" in {
    evaluateExpression(""" to json(["a","b","c"]) """) should returnResult(
      "[\"a\",\"b\",\"c\"]"
    )
  }

  it should "convert a nested list" in {
    evaluateExpression(""" to json([[1,2],3]) """) should returnResult(
      "[[1,2],3]"
    )
  }

  it should "convert a context/map" in {
    evaluateExpression(""" to json({a: 1, b: "foo"}) """) should returnResult(
      """{"a":1,"b":"foo"}"""
    )
  }

  it should "convert a nested context/map" in {
    evaluateExpression(""" to json({person: {name: "Alice", age: 30}}) """) should returnResult(
  ...
  it should "convert a date" in {
    evaluateExpression(""" to json(@"2023-06-14") """) should returnResult(
      "\"2023-06-14\""
    )
  }
  it should "convert a time" in {
    evaluateExpression(""" to json(@"14:55:00") """) should returnResult(
      "\"14:55:00\""
    )
  }
  ...
  it should "convert a day-time duration" in {
    evaluateExpression(""" to json(@"PT2H30M") """) should returnResult(
      "\"PT2H30M\""
    )
  }

  it should "convert a year-month duration" in {
    evaluateExpression(""" to json(@"P1Y6M") """) should returnResult(
      "\"P1Y6M\""
    )
  }

  it should "convert a time with offset" in {
    evaluateExpression(""" to json(@"14:55:00+02:00") """) should returnResult(
      "\"14:55:00+02:00\""
    )
  }

  it should "convert a time with timezone" in {
    // create the time value explicitly to avoid daylight saving issues in the test
  ...
  it should "convert a date and time with offset" in {
    evaluateExpression(
      """ to json(@"2023-06-14T14:55:00+02:00") """
    ) should returnResult(
  ...
  it should "convert a date and time with timezone" in {
    evaluateExpression(
      """ to json(@"2023-06-14T14:55:00@Europe/Berlin") """
    ) should returnResult(
  ...
  it should "convert a range" in {
    evaluateExpression(""" to json((1..10]) """) should returnResult(
      "\"(1..10]\""
    )
  }
