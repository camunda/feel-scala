...
object RangeBuiltinFunction {
  ...
  private def rangeBuiltinFunction(
      params: (String, String),
      invoke: PartialFunction[(Val, Val), Any]
  ) =
    builtinFunction(
      params = List(params._1, params._2),
      invoke = {
    ...
    rangeBuiltinFunction(
      ...
      invoke = {
        ...
        case (point: Val, ValRange(start, _))         =>
          ValBoolean(point < start.value || (point == start.value & !start.isClosed))
    ...
    rangeBuiltinFunction(
      ...
      invoke = {
        ...
        case (ValRange(start, _), point: Val)         =>
          ValBoolean(start.value > point || (start.value == point & !start.isClosed))
        case (point1, point2)                         => ValBoolean(point1 > point2)
      }
    )
  ...
  private def meetsFunction =
    rangeBuiltinFunction(
      ...
      invoke = { case (ValRange(_, end1), ValRange(start2, _)) =>
        ValBoolean(end1.isClosed && start2.isClosed && end1.value == start2.value)
      }
    )
  ...
  private def metByFunction =
    rangeBuiltinFunction(
      params = ("range1", "range2"),
      invoke = { case (ValRange(start1, _), ValRange(_, end2)) =>
        ValBoolean(start1.isClosed && end2.isClosed && start1.value == end2.value)
      }
    )
  ...
  private def overlapsFunction =
    rangeBuiltinFunction(
      params = ("range1", "range2"),
      invoke = { case (ValRange(start1, end1), ValRange(start2, end2)) =>
        ValBoolean(
