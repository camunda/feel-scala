  ...
  private def op(x: Compareable[_], f: T => Boolean): Boolean = x match {
    ...
    case i =>
      throw new IllegalArgumentException(
        s"expect '${value.getClass}' but found '${x.getClass}'")
  }
}

object Compareable {

  implicit class CompareableNumber(val value: Number)
      extends Compareable[Number] {
  ...
  implicit class CompareableLocalTime(val value: LocalTime)
      extends Compareable[LocalTime] {
  ...
  implicit class CompareableLocalDateTime(val value: LocalDateTime)
      extends Compareable[LocalDateTime] {
  ...
  implicit class CompareableDateTime(val value: DateTime)
      extends Compareable[DateTime] {
  ...
  implicit class CompareableYearMonthDuration(val value: YearMonthDuration)
      extends Compareable[YearMonthDuration] {

    def <(x: YearMonthDuration) = value.minus(x).isNegative

    def <=(x: YearMonthDuration) =
      value.minus(x).isNegative || value.minus(x).isZero
  ...
  implicit class CompareableDayTimeDuration(val value: DayTimeDuration)
      extends Compareable[DayTimeDuration] {

    def <(x: DayTimeDuration) = value.minus(x).isNegative

    def <=(x: DayTimeDuration) =
      value.minus(x).isNegative || value.minus(x).isZero
  ...
  implicit class CompareableString(val value: String)
      extends Compareable[String] {
