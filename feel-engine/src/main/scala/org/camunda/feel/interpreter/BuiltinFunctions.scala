...
object BuiltinFunctions extends FunctionProvider {
  ...
  def numberFunction2 = ValFunction(List("from", "grouping separator"), _ match {
  ...
  def numberFunction3 = ValFunction(List("from", "grouping separator", "decimal separator"), _ match {
    case List(ValString(from), ValString(grouping), ValString(decimal)) if (isValidGroupingSeparator(grouping) && isValidDecimalSeparator(decimal) && grouping != decimal) => ValNumber(from.replace(grouping, "").replace(decimal, "."))
  ...
  def substringFunction3 = ValFunction(List("string", "start position", "length"), _ match {
  ...
  def getEntriesFunction = ValFunction(List("context"), _ match {
    case List(ValContext(c: DefaultContext)) => ValList(c.variables.map {
      case (key, value) =>
        ValContext(DefaultContext(variables = Map(
          "key" -> ValString(key),
  ...
  def getValueFunction = ValFunction(List("context", "key"), _ match {
    case List(ValContext(c), ValString(key)) => c.variable(key) match {
