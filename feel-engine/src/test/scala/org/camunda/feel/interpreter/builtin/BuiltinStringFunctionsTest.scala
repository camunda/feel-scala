 ...
 * @author Philipp
 */
class BuiltinStringFunctionsTest extends FlatSpec with Matchers with FeelIntegrationTest {
  ...
  "A split() function" should "return a list of substrings" in {
    ...
    eval(""" split("John Doe", "\s") """) should be(ValList(List(ValString("John"), ValString("Doe"))))

    eval(""" split("a;b;c;;", ";") """) should be(ValList(List(ValString("a"), ValString("b"), ValString("c"), ValString(""), ValString(""))))
  }
