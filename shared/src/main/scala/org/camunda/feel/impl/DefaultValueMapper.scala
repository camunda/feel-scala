...
class DefaultValueMapper extends CustomValueMapper {
  ...
  def toVal(x: Any, innerValueMapper: Any => Val): Option[Val] = x match {
    ...
    case AnEnumeration(x) => Some(ValString(x.toString))
    ...
    case x: java.time.OffsetDateTime =>
      Some(ValDateTime(x.toZonedDateTime()))
    case x: java.time.OffsetTime =>
      Some(ValTime(ZonedTime.of(x)))
    case x: java.util.List[_] =>
      Some(ValList(x.asScala.toList map innerValueMapper))
    case x: java.util.Map[_, _] =>
      Some(ValContext(Context.StaticContext(x.asScala.map {
        case (key, value) =>
          key.toString -> innerValueMapper(value)
      }.toMap)))
    ...
    case x =>
      try {
        Some(syntaxtree.ValContext(Context.CacheContext(ObjectContext(x))))
      } catch {
  ...
  def unpackVal(value: Val, innerValueMapper: Val => Any): Option[Any] =
    value match {
      ...
      case ValContext(c: Context) =>
        Some(c.variableProvider.getVariables.map {
          case (key, value) =>
            value match {
